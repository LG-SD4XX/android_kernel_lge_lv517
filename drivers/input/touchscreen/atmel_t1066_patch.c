...
void mxt_patch_dump_source(struct mxt_data *data, bool do_action)
{
	...
	if (do_action) {
		...
		__mxt_patch_debug(data, "S:%d charging_mode:%d finger_cnt:%d stylus_cnt:%d touch_tot_cnt:%d cur_noise_stage:%d\n",
            		data->patch.src_item[10], data->patch.src_item[11],
			data->patch.src_item[12], data->patch.src_item[13],
			data->patch.src_item[14], data->patch.src_item[15]);

                __mxt_patch_debug(data, "fc_cnt:%d sc_cnt:%d fdc_cnt:%d sdc_cnt:i%d U10:%d U11:%d U12:%d U13:%d U14:%d U15:%d\n",
         		data->patch.src_item[16], data->patch.src_item[17],
			data->patch.src_item[18], data->patch.src_item[19],
			data->patch.src_item[20], data->patch.src_item[21],
			data->patch.src_item[22], data->patch.src_item[23],
			data->patch.src_item[24], data->patch.src_item[25]);

                __mxt_patch_debug(data, "U16:%d U17:%d U18:%d U19:%d U20:%d Charger : %d\n",
			data->patch.src_item[26], data->patch.src_item[27],
			data->patch.src_item[28], data->patch.src_item[29],
			data->patch.src_item[30], data->charging_mode);
	}
}
...
//KHG for debugging
		//__mxt_patch_debug(data, "|- TEST MATCH charging_mode :%d, finger_cnt :%d, stylus_cnt :%d, touch_tot_cnt :%d, cur_noise_stage :%d \n",
		//	data->charging_mode, data->finger_cnt, data->stylus_cnt, data->touch_tot_cnt, data->cur_noise_stage);
//KHG END
...
static void mxt_patch_init_tsrc(struct test_src* tsrc, struct mxt_data *data)
{
//KHG ADD Delete
	//tsrc->charger = -1;
//KHG END
	...
	tsrc->status = -1;
//KHG Modify	
	tsrc->user1 = data->charging_mode;
	tsrc->user2 = data->finger_cnt;
	tsrc->user3 = data->stylus_cnt;
	tsrc->user4 = data->touch_tot_cnt;
	tsrc->user5 = data->cur_noise_stage;
	tsrc->user6 = data->fc_cnt;
	tsrc->user7 = data->sc_cnt;
	tsrc->user8 = data->fdc_cnt;
	tsrc->user9 = data->sdc_cnt;
//KHG END

//KHG ADD	
	tsrc->user10 = t255_user[9];
	tsrc->user11 = t255_user[10];
	tsrc->user12 = t255_user[11];
	tsrc->user13 = t255_user[12];
	tsrc->user14 = t255_user[13];
	tsrc->user15 = t255_user[14];
	tsrc->user16 = t255_user[15];
	tsrc->user17 = t255_user[16];
	tsrc->user18 = t255_user[17];
	tsrc->user19 = t255_user[18];
	tsrc->user20 = t255_user[19];
//KHG END
}
...
//KHG Add
	...
	if (tsrc->user8 >= 0)
		data->patch.src_item[MXT_PATCH_ITEM_USER8] = tsrc->user8;
	if (tsrc->user9 >= 0)
		data->patch.src_item[MXT_PATCH_ITEM_USER9] = tsrc->user9;
	if (tsrc->user10 >= 0)
		data->patch.src_item[MXT_PATCH_ITEM_USER10] = tsrc->user10;
	if (tsrc->user11 >= 0)
		data->patch.src_item[MXT_PATCH_ITEM_USER11] = tsrc->user11;
	if (tsrc->user12 >= 0)
		data->patch.src_item[MXT_PATCH_ITEM_USER12] = tsrc->user12;
	if (tsrc->user13 >= 0)
		data->patch.src_item[MXT_PATCH_ITEM_USER13] = tsrc->user13;
	if (tsrc->user14 >= 0)
		data->patch.src_item[MXT_PATCH_ITEM_USER14] = tsrc->user14;
	if (tsrc->user15 >= 0)
		data->patch.src_item[MXT_PATCH_ITEM_USER15] = tsrc->user15;
	if (tsrc->user16 >= 0)
		data->patch.src_item[MXT_PATCH_ITEM_USER16] = tsrc->user16;
	if (tsrc->user17 >= 0)
		data->patch.src_item[MXT_PATCH_ITEM_USER17] = tsrc->user17;
	if (tsrc->user18 >= 0)
		data->patch.src_item[MXT_PATCH_ITEM_USER18] = tsrc->user18;
	if (tsrc->user19 >= 0)
		data->patch.src_item[MXT_PATCH_ITEM_USER19] = tsrc->user19;
	if (tsrc->user20 >= 0)
		data->patch.src_item[MXT_PATCH_ITEM_USER20] = tsrc->user20;
//KHG END
