...
int sw49105_sic_abt_is_set_func(void)
{
...
static uint32_t abt_ksocket_rcv_from_pctool(uint8_t *buf, uint32_t len)
{
	...
	if (abt_comm.recv_packet->hdr.cmd == TCP_REG_READ) {
		ret = sw49105_reg_read(abt_comm.dev,
		...
		ret = sw49105_reg_write(abt_comm.dev,
					abt_comm.recv_packet->hdr.addr,
...
static uint32_t abt_ksocket_rcv_from_abtstudio(uint8_t *buf, uint32_t len)
{
	...
	case CMD_RAW_DATA_COMPRESS_WRITE:
		sw49105_write_value(abt_comm.dev,
...
void sw49105_sic_abt_probe(void)
{
	mutex_init(&abt_comm_lock);
}

void sw49105_sic_abt_remove(void)
{
...
void sw49105_sic_abt_init(struct device *dev)
{
	u32 head_loc[4];
	...
	sw49105_read_value(dev, CMD_ABT_LOC_X_START_READ, &head_loc[0]);
	sw49105_read_value(dev, CMD_ABT_LOC_X_END_READ, &head_loc[1]);
	sw49105_read_value(dev, CMD_ABT_LOC_Y_START_READ, &head_loc[2]);
	sw49105_read_value(dev, CMD_ABT_LOC_Y_END_READ, &head_loc[3]);

	memset(&tHeadBuffer, 0x00, sizeof(struct T_ABTLog_FileHead));
...
int sw49105_sic_abt_is_debug_mode(void)
{
...
int sw49105_sic_abt_irq_handler(struct device *dev)
{
	struct sw49105_data *d = to_sw49105_data(dev);
	int ret = 0;
	u8 all_data[264];

	int report_mode = sw49105_sic_abt_is_set_func();
	if (sw49105_reg_read(dev, tc_ic_status,
	...
	if (report_mode) {
		...
		sw49105_sic_abt_report_mode(dev, all_data);
	} else {
		if (d->info.wakeup_type == ABS_MODE) {
			if (sw49105_sic_abt_is_debug_mode()) {
				sw49105_sic_abt_onchip_debug(dev, &all_data[8]);
				ret = sw49105_irq_abs(dev);
			} else {
				sw49105_sic_abt_ocd_off(dev);
				ret = sw49105_irq_abs(dev);
			}
		} else {
			ret = sw49105_irq_lpwg(dev);
...
void sw49105_sic_abt_ocd_off(struct device *dev)
{
	u32 wdata = 0;

	if (abt_ocd_off) {
		sw49105_write_value(dev, CMD_ABT_OCD_ON_WRITE, wdata);
		sw49105_read_value(dev, CMD_ABT_OCD_ON_READ, &wdata);
		TOUCH_I("[ABT] onchipdebug off: wdata=%d\n", wdata);
		abt_ocd_off = 0;
	}
}

void sw49105_sic_abt_onchip_debug(struct device *dev, u8 *all_data)
{
	u32			i, j;
	u32			wdata;
		...
		if (u8_ocd_pieces_cnt != prev_rnd_piece_no) {
			if (u8_ocd_pieces_cnt != DEF_RNDCPY_EVERY_Nth_FRAME - 1) {
				...
				memcpy(&abt_reportP[34 * 4], &all_data[34 * 4],
					sizeof(u8) * 112);
				sw49105_reg_write(dev, tc_interrupt_status, &u32_rst_offset_val,
					sizeof(u32_rst_offset_val));
			}
		}
	...
	u16 nOffsetValueByteSize, u8 *pData, u32 nSize)
{
	...
	while (nRead > 0) {
		sw49105_reg_write(dev, offset_addr, (u8 *)&nOffset,
			nOffsetValueByteSize);
		nRead = sw49105_reg_read(dev, nMemAddr, pData, nRead);
		if (nRead <= 0)
			return nRead;
...
void sw49105_sic_abt_report_mode(struct device *dev, u8 *all_data)
{
	...
	struct	sw49105_touch_info *t_info = (struct sw49105_touch_info *) all_data;
	u32 	u32_rst_offset_val	= 1;
		...
		if (t_info->data[0].track_id != 15) {
			if (abt_report_point) {
				...
				memcpy(d_data_ptr, &t_info->data[0],
					sizeof(struct sw49105_touch_data) * t_info->touch_cnt);
				d_data_ptr += sizeof(struct sw49105_touch_data)
					* t_info->touch_cnt;
			} else
				packet_ptr->touchCnt = 0;
		}
...
static struct attribute *sw49105_abt_attribute_list[] = {
	&touch_attr_abt_monitor.attr,
	&touch_attr_raw_report.attr,
	NULL,
};

static const struct attribute_group sw49105_abt_attribute_group = {
	.attrs = sw49105_abt_attribute_list,
};

void sw49105_sic_abt_register_sysfs(struct kobject *kobj)
{
	int ret = sysfs_create_group(kobj, &sw49105_abt_attribute_group);
