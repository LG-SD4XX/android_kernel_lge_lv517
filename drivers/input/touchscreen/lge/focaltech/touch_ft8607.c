 ...
 */
#include "touch_ft8607.h"
#include "touch_ft8607_prd.h"
...
#ifdef FT8607_ESD_SKIP_WHILE_TOUCH_ON
static int finger_cnt = 0;

bool ft8607_check_finger(void)
{
	return finger_cnt==0? false:true;
}
EXPORT_SYMBOL(ft8607_check_finger);
#endif
...
static struct ft8707_ic_info *ext_ic_info;

int ft8607_chipid_info(void)
{
	int ret = 0;
...
#if 0
		data = 0x0A;
		ret |= ft8707_reg_write(dev, 0x80, &data, 1);	// Touch Differ Threshold (32 * 0x0A = 320)
		data = 0x21;
		ret |= ft8707_reg_write(dev, 0x89, &data, 1);	// Frame Rate for LPWG (30 Hz)
#endif
		data = 0x28;
		ret |= ft8707_reg_write(dev, 0xB4, &data, 1);	// Active Area LSB of X1
		data = 0x00;
		ret |= ft8707_reg_write(dev, 0xB5, &data, 1);	// Active Area MSB of X1 (40)
		data = 0xA8;
		ret |= ft8707_reg_write(dev, 0xB6, &data, 1);	// Active Area LSB of X2
		data = 0x02;
		ret |= ft8707_reg_write(dev, 0xB7, &data, 1);	// Active Area MSB of X2 (680)
		data = 0x22;
		ret |= ft8707_reg_write(dev, 0xB8, &data, 1);	// Active Area LSB of Y1
		data = 0x00;
		ret |= ft8707_reg_write(dev, 0xB9, &data, 1);	// Active Area MSB of Y1 (34)
		data = 0xDE;
		ret |= ft8707_reg_write(dev, 0xBA, &data, 1);	// Active Area LSB of Y2
		data = 0x04;
		ret |= ft8707_reg_write(dev, 0xBB, &data, 1);	// Active Area MSB of Y2 (1246)
		break;
...
int ft8707_lpwg_reset(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);

	TOUCH_I("ft8707_lpwg_reset\n");
	touch_gpio_direction_output(ts->reset_pin, 0);
	touch_msleep(5);
	touch_gpio_direction_output(ts->reset_pin, 1);
		...
		if (screen_mode == 0) {
			if((ts->lpwg.mode == 0) && !ts->role.mfts_lpwg) {
				...
				TOUCH_I("3\n");
					...
					touch_msleep(200);
#endif
				ret = ft8707_lpwg_control(dev, ts->lpwg.mode); // LCD SLP In or LCD Deep Sleep, 100ms -> Touch LPWG
			}
			else {
				next_state = TC_STATE_DEEP_SLEEP;
				ret = ft8707_deep_sleep(dev, 1); // LCD Deep Sleep, 100ms -> Touch Deep Sleep, DSV Off
				TOUCH_I("4\n");
#if 0
				TOUCH_I("4 Call DSV OFF\n");
				LCD_DSV_OFF;
#endif
			}
		}
...
static void ft8707_connect(struct device *dev)
{
	...
	ft8707_reg_write(dev, SPR_CHARGER_STS, &d->charger, sizeof(u8));
}
...
/*	case NOTIFY_QUICKCOVER_STATE:
		...
		ret = ft8707_reg_write(dev, SPR_QUICKCOVER_STS, (u8 *)data, sizeof(u8));
		break; */
	default:
...
//	ft8707_init_works(d);
	...
	d->tci_debug_type = TCI_DEBUG_BUFFER_ALWAYS;
	atomic_set(&ts->state.debug_option_mask, DEBUG_OPTION_2);
...
//////////////////////////////////////////////
	...
	if ((retrim_val[0]==(~retrim_val[1]&0xFF))&&(retrim_val[2]==(~retrim_val[3]&0xFF))) {
		...
		ret = ft8707_reg_write(dev, 0xBF, i2c_buf, 4 + 5);
		if(ret < 0) {
...
#endif
	ret = ft8707_reg_write(dev, SPR_CHARGER_STS, &d->charger, sizeof(u8));
	if (ret)
		TOUCH_E("failed to write \'spr_charger_sts\', ret:%d\n", ret);

	ret = ft8707_reg_write(dev, SPR_QUICKCOVER_STS, &ts->lpwg.qcover, sizeof(u8));
	if (ret)
...
#ifdef POINTBUF_CRC
	u8 point_buf[POINT_READ_BUF + 2] = { 0, };
	u8 xor_data;
	int i;
#else
...
#ifdef POINTBUF_CRC
	ret = ft8707_reg_read(dev, 0x02, point_buf+2, POINT_READ_BUF);
#else
...
#ifdef POINTBUF_CRC
	if((point_buf[2] == 0x01) && (point_buf[9] != 0xff)) {
		xor_data = point_buf[2];
		for(i = 0; i < 9; i++) {
			xor_data ^= point_buf[3+i];
		}

		if(xor_data != point_buf[POINT_READ_BUF]) {
			TOUCH_I(">>diff  xor_data:%02x, fw_data[63]:%02x, fw_data[64]:%02x\n", 
				xor_data, point_buf[POINT_READ_BUF], point_buf[POINT_READ_BUF + 1]);
			return -ERANGE;
		}
	}
#endif
...
static int ft8707_get_cmd_version(struct device *dev, char *buf)
{
	...
	if (d->ic_info.chip_id == 0x86) {
		offset += snprintf(buf + offset, PAGE_SIZE - offset, "Product-id : [FT860%x]\n\n", d->ic_info.chip_id_low);
	}
	else {
...
static bool ft8607_get_device_type(void)
{
	bool bdevice = false;
...
static int __init touch_device_init(void)
{
	...
	if (!ft8607_get_device_type()) {
		TOUCH_I("%s, ft8607 not found\n", __func__);
		return 0;
	}
