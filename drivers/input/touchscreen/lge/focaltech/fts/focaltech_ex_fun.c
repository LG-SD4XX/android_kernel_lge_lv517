...
struct i2c_client *fts_client = NULL;
...
static ssize_t fts_debug_write(struct file *filp, const char __user *buff, size_t count, loff_t *ppos)
{
	...
	if (copy_from_user(&writebuf, buff, buflen)) {
		dev_err(&fts_client->dev, "[FTS] %s:copy from user error\n", __func__);
		return -EFAULT;
	}
	...
	case PROC_UPGRADE:
		{
			...
			printk("[Touch][FTS] %s\n", upgrade_file_path);
			disable_irq(fts_client->irq);
			#if GTP_ESD_PROTECT
			apk_debug_flag = 1;
			#endif

//			ret = fts_ctpm_fw_upgrade_with_app_file(fts_client, upgrade_file_path);
			#if GTP_ESD_PROTECT
			apk_debug_flag = 0;
			#endif
			enable_irq(fts_client->irq);
			if (ret < 0) {
				dev_err(&fts_client->dev, "[FTS] %s:upgrade failed.\n", __func__);
				return ret;
			}
		}
		break;
	case PROC_READ_REGISTER:
		writelen = 1;
		ret = fts_i2c_write(fts_client, writebuf + 1, writelen);
		if (ret < 0) {
			dev_err(&fts_client->dev, "[FTS] %s:write iic error\n", __func__);
			return ret;
		}
		break;
...
static ssize_t fts_debug_read(struct file *filp, char __user *buff, size_t count, loff_t *ppos)
{
	...
	case PROC_UPGRADE:
		...
		regaddr = 0xA6;
		ret = fts_read_reg(fts_client, regaddr, &regvalue);
		if (ret < 0)
	...
	case PROC_READ_REGISTER:
		readlen = 1;
		ret = fts_i2c_read(fts_client, NULL, 0, buf, readlen);
		if (ret < 0) {
			dev_err(&fts_client->dev, "[FTS] %s:read iic error\n", __func__);
			return ret;
		}
		num_read_chars = 1;
		break;
	...
	if (copy_to_user(buff, buf, num_read_chars)) {
		dev_err(&fts_client->dev, "[FTS] %s:copy to user error\n", __func__);
		return -EFAULT;
	}
...
int fts_create_apk_debug_channel(struct i2c_client * client)
{
	...
	{
		dev_err(&client->dev, "[FTS] Couldn't create proc entry!\n");

		return -ENOMEM;
	}
	else
	{
		dev_info(&client->dev, "[FTS] Create proc entry success!\n");
...
static ssize_t fts_tpfwver_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if (fts_read_reg(fts_client, FTS_REG_FW_VER, &fwver) < 0)
		return -1;
...
static ssize_t fts_tprwreg_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (num_read_chars != 2)
	{
		if (num_read_chars != 4)
		{
			dev_err(dev, "[FTS] please input 2 or 4 character\n");
			goto error_return;
		}
	}
	...
	if (0 != retval)
	{
		dev_err(dev, "[FTS] %s() - ERROR: Could not convert the given input to a number. The given input was: \"%s\"\n", __FUNCTION__, buf);
		goto error_return;
	}
	if (2 == num_read_chars)
	{
		/*read register*/
		regaddr = wmreg;
		printk("[FTS][test](0x%02x)\n", regaddr);
		if (fts_read_reg(client, regaddr, &regvalue) < 0)
			printk("[FTS] %s : Could not read the register(0x%02x)\n", __func__, regaddr);
		else
			printk("[FTS] %s : the register(0x%02x) is 0x%02x\n", __func__, regaddr, regvalue);
	}
	else
	{
		regaddr = wmreg>>8;
		...
		if (fts_write_reg(client, regaddr, regvalue)<0)
			dev_err(dev, "[FTS] %s : Could not write the register(0x%02x)\n", __func__, regaddr);
		else
			dev_dbg(dev, "[FTS] %s : Write 0x%02x into register(0x%02x) successful\n", __func__, regvalue, regaddr);
	}
	error_return:
	mutex_unlock(&g_device_mutex);
...
//		uc_host_fm_ver = fts_ctpm_get_i_file_ver();
		dev_dbg(dev, "[FTS] %s [FTS] upgrade to new version 0x%x\n", __func__, uc_host_fm_ver);
	}
	else
	{
		dev_err(dev, "[FTS] %s ERROR:[FTS] upgrade failed ret=%d.\n", __func__, i_ret);
	}
