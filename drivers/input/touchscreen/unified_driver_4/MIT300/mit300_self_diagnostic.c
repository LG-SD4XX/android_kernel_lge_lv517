...
ssize_t MIT300_GetTestResult(struct i2c_client *client, char *pBuf, int *result, int type)
{
	...
	case RAW_DATA_STORE:
		...
		if (bootmode == BOOT_MINIOS)
			sprintf(data_path, "/data/logger/%s_rawdata.csv", temp_buf);
		else if (bootmode == BOOT_NORMAL)
			sprintf(data_path, "/sdcard/%s_rawdata.csv", temp_buf);
		...
		TOUCH_LOG("write log %s_rawdata.csv\n", temp_buf);
		if (fd >= 0) {
	...
	case OPENSHORT_STORE:
		snprintf(temp_buf, strlen(pBuf), "%s", pBuf);
		if (bootmode)
			sprintf(data_path, "/data/logger/%s_openshort.csv", temp_buf);
		else
	...
	case CM_DELTA_STORE:
		snprintf(temp_buf, strlen(pBuf), "%s", pBuf);
		if (bootmode)
			sprintf(data_path, "/data/logger/%s_delta.csv", temp_buf);
		else
			sprintf(data_path, "/sdcard/%s_delta.csv", temp_buf);
		ret += MIT300_PrintData(client, read_buf, result, type);
		if (ret < 0) {
			TOUCH_ERR("fail to print type data\n");
			goto ERROR;
	...
	case CM_JITTER_STORE:
		snprintf(temp_buf, strlen(pBuf), "%s", pBuf);
		if (bootmode)
			sprintf(data_path, "/data/logger/%s_jitter.csv", temp_buf);
		else
			sprintf(data_path, "/sdcard/%s_jitter.csv", temp_buf);
		ret += MIT300_PrintData(client, read_buf, result, type);
		if (ret < 0) {
			TOUCH_ERR("fail to print type data\n");
			goto ERROR;
