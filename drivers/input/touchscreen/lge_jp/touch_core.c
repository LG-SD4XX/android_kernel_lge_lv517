...
irqreturn_t touch_irq_handler(int irq, void *dev_id)
{
	...
	if (atomic_read(&ts->state.fb) == FB_SUSPEND) { // To prevent from i2c xfer on msm-i2c-rt-pm-suspended (irq thread can run in entering suspend)
		wake_lock_timeout(&ts->lpwg_wake_lock, msecs_to_jiffies(2000));
	}
