...
static void msm_otg_start_host(struct usb_otg *otg, int on)
{
	...
	if (on) {
		...
		if ((motg->epack_state <= EPACK_POWER_LOW) &&
				(next_ep_state == EPACK_POWER_OK)) {
			...
			motg->epack_state = next_ep_state;
		}
#endif
#ifdef CONFIG_LGE_USB_TYPE_A
...
#ifdef CONFIG_INPUT_EPACK
		next_ep_state = get_epack_status();
		if ((motg->epack_state >= EPACK_POWER_OK) &&
				(next_ep_state <= EPACK_POWER_LOW)) {
			pr_err("[EPACK-OTG] epack detached. switch data line\n");
			if ((motg->pdata->dpdm_sw_reverse) ||
					lge_get_board_revno() == HW_REV_A_2)
				gpio_set_value(motg->pdata->dpdm_sw_gpio, 1);
			else
...
static void msm_hsusb_vbus_power(struct msm_otg *motg, bool on)
{
	...
	if (get_epack_status() >= EPACK_POWER_OK) {
		pr_info("epack connected. do nothing\n");
		return;
	}
#endif
...
#ifdef CONFIG_INPUT_EPACK
		if (get_epack_status() == EPACK_POWER_OK){
			dev_info(&motg->pdev->dev, "epack power ok, otg start\n");
...
#ifdef CONFIG_INPUT_EPACK
		if (test_bit(ID, &motg->inputs) && get_epack_status() <= EPACK_POWER_LOW) {
#else
				  ...
				  const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_USB_EPACK:
		next_ep_state = get_epack_status();
		pr_err("[EPACK-OTG] next_ep_state = %d, motg->epack_state = %d\n",
				next_ep_state, motg->epack_state);
		if (next_ep_state == motg->epack_state)
			break;
		if (((next_ep_state == EPACK_POWER_OK) &&
				(motg->epack_state <= EPACK_POWER_LOW)) ||
				((next_ep_state <= EPACK_POWER_LOW) &&
				(motg->epack_state == EPACK_POWER_OK))) {
			msm_otg_kick_sm_work(motg);
		}
		break;
#endif
	...
	case POWER_SUPPLY_PROP_PRESENT:
#ifdef CONFIG_INPUT_EPACK
		pr_err("[EPACK-OTG] present : %d, vbus_source : %d\n",
				val->intval, get_vbus_source());
		if (get_epack_status() == EPACK_POWER_OK)
			break;
#endif
