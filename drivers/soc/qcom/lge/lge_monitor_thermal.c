...
#define VTS_WEIGHT_CASE_TEMP	(40)
...
static void _poll_monitor(struct lge_monitor_thermal_data *monitor_dd)
{
	...
	if (!IS_ERR(monitor_dd->vadc_dev)) {
		...
		if (rc)
			...
			monitor_dd->last_case_temp = (unsigned int)result.physical;
		...
		if (monitor_dd->tz_vts)
			...
			monitor_dd->last_vts_temp =
				...
				VTS_WEIGHT_CASE_TEMP * monitor_dd->last_case_temp)
				/ VTS_WEIGHT_BY_PERCENT;

		pr_info("[TM][I] XO=%d, QUIET=%d, VTS=%lu\n",
					monitor_dd->last_xo_temp,
					monitor_dd->last_case_temp,
					monitor_dd->last_vts_temp);
	}
}
