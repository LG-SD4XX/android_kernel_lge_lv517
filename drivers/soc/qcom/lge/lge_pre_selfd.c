...
static char pre_selfd_power_buf[4096];
static char pre_selfd_pass_buf[4096];
unsigned int selfd_cnt= 0;
unsigned int selfd_power_cnt = 0;
...
/************************** PRE SELFD POWER Test CASE ********************************/

static ssize_t power_test_store(struct device *dev, struct device_attribute *attr,
        const char *buf, size_t count)
{
    ...
    if ( buf[0] =='$' )
    {
        memset(pre_selfd_power_buf, 0, 4096); selfd_power_cnt = 0;
        return -EINVAL;
    }
    str_len = sprintf(str, "%s", buf);
    if ( (selfd_power_cnt+str_len) > 4096 )
    {
        sprintf((char *) &pre_selfd_power_buf[selfd_power_cnt], "$\n");
        selfd_power_cnt++;
        return selfd_power_cnt;
    }

    sprintf((char *) &pre_selfd_power_buf[selfd_power_cnt], "%s", buf);
    selfd_power_cnt += str_len;

    return selfd_power_cnt;
}

static ssize_t power_test_show(struct device *dev, struct device_attribute *attr,
        char *buf)
{
    ssize_t ret_len = 0;
    ret_len = snprintf(buf, PAGE_SIZE, "%s", pre_selfd_power_buf);
    return ret_len;

}

static DEVICE_ATTR(power_test, 0644, power_test_show, power_test_store);
...
static int pre_selfd_ctrl_probe(struct platform_device *pdev)
{
    ...
    rc2 = device_create_file(&pdev->dev, &dev_attr_power_test);
