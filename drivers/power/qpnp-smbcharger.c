...
#define PARALLEL_REENABLE_TIMER_MS	1000
		...
		defined(CONFIG_MACH_MSM8940_SF3_MPCS_US) || defined(CONFIG_MACH_MSM8940_SF3_TMO_US)
...
#if defined(CONFIG_MACH_MSM8940_SF3_MPCS_US) || defined(CONFIG_MACH_MSM8940_SF3_TMO_US)
		...
		int total_current_ma)
{
	...
	if ((lge_get_boot_mode() != LGE_BOOT_MODE_CHARGERLOGO) && 
			chip->qc20.status){
		total_current_ma = max(target_icl_ma, total_current_ma);
...
#elif defined(CONFIG_MACH_MSM8940_SF3_MPCS_US) || defined(CONFIG_MACH_MSM8940_SF3_TMO_US)
	if ((lge_get_boot_mode() != LGE_BOOT_MODE_CHARGERLOGO) && chip->qc20.status)
		new_parallel_cl_ma = total_current_ma
				* (100 - (smbchg_main_chg_icl_percent - 10)) / 100;
	else
		new_parallel_cl_ma = total_current_ma
				* (100 - smbchg_main_chg_icl_percent) / 100;
#else
...
#ifdef	CONFIG_INPUT_EPACK
	char *vbus_source = (get_vbus_source()==FROM_EPACK)?"->Epack":" ";
