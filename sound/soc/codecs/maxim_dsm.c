...
void maxdsm_update_param(void)
{
	...
	case PLATFORM_TYPE_B:
		maxdsm.filter_set = DSM_ID_FILTER_GET_AFE_PARAMS;
		maxdsm_dsm_open(&maxdsm);
		maxdsm.filter_set = DSM_ID_FILTER_GET_AFE_LOG_PARAMS;
		maxdsm_dsm_open(&maxdsm);
...
int maxdsm_get_registered_status(void)
{
	return maxdsm.registered;
}
EXPORT_SYMBOL_GPL(maxdsm_get_registered_status);
		...
		size_t count, loff_t *ppos)
{
	...
	ret = copy_to_user(buf, maxdsm.param,
                count > maxdsm.param_size * sizeof(uint32_t) ?
                    maxdsm.param_size * sizeof(uint32_t) : count);
	if (ret)
